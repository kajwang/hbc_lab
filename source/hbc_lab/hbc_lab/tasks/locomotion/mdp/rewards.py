...
def track_lin_vel_xy_yaw_frame_exp(
    ...
    """Reward xy linear velocity tracking in the gravity-aligned robot yaw frame."""
...
def track_ang_vel_z_exp(
    ...
    """Reward yaw angular velocity tracking using an exponential kernel."""
    asset: RigidObject = env.scene[asset_cfg.name]
    ang_vel_error = torch.square(env.command_manager.get_command(command_name)[:, 2] - asset.data.root_ang_vel_b[:, 2])
    return torch.exp(-ang_vel_error / std**2)


def _body_pose_command_position_distance(
    ...
    current_pos_w = asset.data.body_pos_w[:, asset_cfg.body_ids[0], :3]
...
def _body_pose_command_position_distance_w(
    env: ManagerBasedRLEnv,
    command_name: str,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
) -> torch.Tensor:
...
def body_pose_command_position_error_l2(
    ...
    """Penalize Euclidean body-position error to a UniformPoseCommand target."""
    return _body_pose_command_position_distance(env, command_name, asset_cfg)


def body_pose_command_position_error_exp(
    env: ManagerBasedRLEnv,
    command_name: str,
    std: float,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
) -> torch.Tensor:
    """Reward body-position tracking with a broad squared exponential kernel."""
    distance = _body_pose_command_position_distance(env, command_name, asset_cfg)
    return torch.exp(-torch.square(distance) / std**2)


def body_pose_command_position_error_tanh(
    env: ManagerBasedRLEnv,
    command_name: str,
    std: float,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
) -> torch.Tensor:
    """Reward body-position tracking with a fine tanh kernel."""
    distance = _body_pose_command_position_distance(env, command_name, asset_cfg)
    return 1.0 - torch.tanh(distance / std)


def body_pose_command_position_error_w_l2(
    env: ManagerBasedRLEnv,
    command_name: str,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
) -> torch.Tensor:
    """Penalize body-position error to a command term's world-frame target."""
    return _body_pose_command_position_distance_w(env, command_name, asset_cfg)


def body_pose_command_position_error_w_exp(
    env: ManagerBasedRLEnv,
    command_name: str,
    std: float,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
) -> torch.Tensor:
    """Reward body-position tracking against a command term's world-frame target."""
    distance = _body_pose_command_position_distance_w(env, command_name, asset_cfg)
    return torch.exp(-torch.square(distance) / std**2)


def body_pose_command_position_error_w_tanh(
    env: ManagerBasedRLEnv,
    command_name: str,
    std: float,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
) -> torch.Tensor:
    """Reward fine body-position tracking against a command term's world-frame target."""
    distance = _body_pose_command_position_distance_w(env, command_name, asset_cfg)
    return 1.0 - torch.tanh(distance / std)
...
def body_pose_command_orientation_error_l2(
    env: ManagerBasedRLEnv,
    command_name: str,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
) -> torch.Tensor:
    """Penalize shortest-path orientation error to a UniformPoseCommand target."""
    return _body_pose_command_orientation_distance(env, command_name, asset_cfg)


def body_pose_command_orientation_error_tanh(
    env: ManagerBasedRLEnv,
    command_name: str,
    std: float,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
) -> torch.Tensor:
    """Reward body-orientation tracking with a broad tanh kernel."""
    distance = _body_pose_command_orientation_distance(env, command_name, asset_cfg)
    return 1.0 - torch.tanh(distance / std)


def body_pose_command_orientation_error_w_tanh(
    env: ManagerBasedRLEnv,
    command_name: str,
    std: float,
    asset_cfg: SceneEntityCfg = SceneEntityCfg("robot"),
...
def feet_slide(env: ManagerBasedRLEnv, sensor_cfg: SceneEntityCfg, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    """Penalize foot sliding while the selected foot bodies are in contact."""
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
