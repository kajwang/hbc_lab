...
- [x] Add a failing test that requires `format_shape_assignment(ALL_SHAPE_NAMES)` to list environments 0 through 10, label the final three entries as OOD, and preserve the exact asset order.
- [x] Add static assertions that the play scene contains 11 environments at 6 m spacing and that the duplicate `@configclass` decorator is absent.
- [x] Run `pytest -q tests/test_g1_dex1_multishape_bps.py` and confirm the new assertions fail.
- [x] Implement:
...
- [x] Set `G1Dex1HierDrcMultiShapePlayEnvCfg.scene` and `self.scene.num_envs` to 11, use 6 m spacing, and print the mapping once in `__post_init__`.
- [x] Run `pytest -q tests/test_g1_dex1_multishape_bps.py` and confirm it passes.
...
- [x] Add failing static tests requiring both launch names, 11 environments, fixed right-hand evaluation, 6 m spacing, BPS toggles, and the two stable `model_11300.pt` paths.
- [x] Run `pytest -q tests/test_g1_dex1_multishape_bps.py` and confirm the launch assertions fail.
- [x] Replace the old generic play entries with:
...
- [x] Update the design document checkpoint contract from `model_10500.pt` to the verified latest common `model_11300.pt`.
- [x] Parse `.vscode/launch.json` as JSON-with-comments through the existing static test source checks and rerun the focused test.
...
- [x] Copy the BPS checkpoint from:
...
- [x] Copy the no-BPS checkpoint from:
...
- [x] Compare remote and local SHA-256 hashes for both files.
- [x] Load each checkpoint with `torch.load(..., map_location="cpu", weights_only=False)` and verify the first actor layer expects 1552 inputs for BPS and 1360 for no-BPS.
- [x] Run `pytest -q tests/test_g1_dex1_multishape_bps.py tests/test_g1_dex1_hier_drc_static.py`.
- [x] Run `python -m compileall -q source/hbc_lab/hbc_lab/tasks/manager_based/skill/g1_dex1_hier_drc`.
- [x] If local Isaac Sim can launch without monopolizing the user's display, run one short Play smoke test per entry; otherwise report that visual runtime verification remains for the user.
